...
Le body du 17 août 2026 est **périmé**. Statut à la **v0.8** (plus le correctif TE×10 du 2026-09-09) :

| # | Demande | v0.8 |
|---|---------|------|
| 1 | Titley Swift / Ranger, rename + découpe 5 s | **Livré** 0.7.0 (noms). **Corrigé** 2026-09-09 : tout le son, pas seulement 5 s (Rust + `Data_k` tronqué). |
| 2 | Plusieurs exe en parallèle | **Non.** Mode Batch (une instance). D14. |
| 3 | Valider : tri colonnes | **Livré** 0.7.0 |
| 4 | Valider : filtre taxon observateur | **Livré** 0.7.0 |
...
| #10 | Activité lente (rescan xlsx à chaque case), filtres morts, relecture du `_Vu` limitée aux taxons écoutés, un seul carré | **Livré** : cache RAM ; cases = recalcul mémoire ; `_Vu` racine de session et `Data_k/` ; listes de filtres depuis l'univers (les autres carrés restent) ; 1er scan sans descendre dans les WAV. |
...
### 0.6 Batch Préparer : dernières nuits « OK » sans renommage

**Interne.** Pas d'issue GitHub. Cible exe **0.8.2**.

Retour collègue (2026-09-19) : ~15 nuits en **batch ▶ Préparer**. Les 10–12
...
| Dossiers déjà en cours | Pas de migration. Manifest, registre (schéma 3), Suivi Excel, WAV et participations restent. Un `Data_k` déjà présent ne ressort pas « incomplet » s'il manque des tranches de 5 s. Nuit déjà renommée et expansée : non refaite. État TERMINE ou FINI : pas de listing WAV, pas de `trigger_compute`. |
